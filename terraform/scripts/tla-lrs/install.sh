...
echo "Creating keycloak client for tla lrs"
...
-H "Authorization: bearer ${KEYCLOAK_TOKEN}" \
-X GET localhost:8090/admin/realms/cnoe/clients | jq -e -r  '.[] | select(.clientId == "tla-lrs") | .id')
...
echo 'storing client secrets to tla-lrs namespace'

envsubst < secret-sso.yaml | kubectl apply -f -


# If TLS secret is available in /private, use it. Could be empty... ( need to confirm if this is required for tla lrs)
