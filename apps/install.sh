...
do
  ...
  -H "Authorization: bearer ${KEYCLOAK_TOKEN}" \
  -X GET localhost:8090/admin/realms/cnoe/clients | jq -e -r  '.[] | select(.clientId ==' + $app_name + ') | .id')
  ...
  echo 'storing client secrets to ' + $app_name + ' namespace'
